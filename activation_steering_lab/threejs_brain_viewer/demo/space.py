...
```bash
pip install gradio_threejs_brain_viewer
```
...
""", elem_classes=["md-custom"], header_links=True)
    ...
    gr.ParamViewer(value=_docs["threejs_brain_viewer"]["members"]["__init__"], linkify=[])


    gr.Markdown("### Events")
    gr.ParamViewer(value=_docs["threejs_brain_viewer"]["events"], linkify=['Event'])
...
def predict(
    ...
    return value
```
""", elem_classes=["md-custom", "threejs_brain_viewer-user-fn"], header_links=True)
